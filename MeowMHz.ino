// ============================================================================
//  MM (Meow MHz) — CyberMeow sub-GHz handheld        [prototype v7 -> toward 1.0]
...
// ---------- screens ----------
enum Screen   { SCR_MENU, SCR_READ, SCR_ANALYZE, SCR_XMIT, SCR_SAVED };
...
void drawMenu() {
  chrome();
  const char *items[MENU_N] = { "READ", "ANALYZE", "TRANSMIT", "SAVED" };
  for (int i = 0; i < MENU_N; i++) {
    int y = 30 + i * 22;
    if (i == menuSel) { tft.fillRect(8, y, 148, 20, C_GRN); tft.setTextColor(C_BLACK); }
    else              { tft.drawRect(8, y, 148, 20, C_GRN_M); tft.setTextColor(C_GRN); }
    tft.setTextSize(2); tft.setCursor(14, y + 3); tft.print(items[i]);
  }
  tft.drawRect(CAT_X, CAT_Y, CAT_W, CAT_H, C_GRN_M);
  animateCat();
  tft.setTextSize(1);
  tft.setTextColor(ccOK ? C_GRN : C_GRN_D);
  tft.setCursor(CAT_X + 8, CAT_Y + CAT_H + 6); tft.print(ccOK ? "RF OK" : "RF --");
  hintLine("w/s move   e select");
}
